...
bool test(float x, float y) {
	...
	const float my_res = float_utils::mul<rounding_mode>(x, y);
	...
	std::cout <<
		"Hardware mul: " << std::hex << hw_bin << std::dec << "  " << std::hexfloat << hw_res << "\n" <<
		"      My mul: " << std::hex << my_bin << std::dec << "  " << std::hexfloat << my_res << "\n";
